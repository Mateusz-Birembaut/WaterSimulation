...
void main() {
    ...
    if (uv.x < 1.0 && uv.y < 1.0) {
        ...
        fragColor = vec4(0.0, abs(texture(debugTexture, vec2(uv.x - 1.0, uv.y)).g), 0.0, 1.0);
    } else if (uv.x < 1.0 && uv.y >= 1.0) {
        fragColor = vec4(0.0, 0.0, abs(texture(debugTexture, vec2(uv.x, uv.y - 1.0)).b), 1.0);
    } else {
        fragColor = vec4(abs(texture(debugTexture2, vec2(uv.x - 1.0, uv.y - 1.0)).a));
    }
}
