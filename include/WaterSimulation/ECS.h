...
namespace WaterSimulation{
	...
	const std::uint32_t MAX_ENTITIES = 2048; //ne pas mettre valeur max uint32 utilser au maximum max(uint32) -1
	const std::uint32_t INVALID = MAX_ENTITIES - 1; //permet de savoir si un composant est valide (cad contient un composant pour une certaine entity)
	...
	class ComponentStorage : public IComponentStorage{ //stocke les composants pour un unique type de composant 
		...
		std::vector<uint32_t> sparse; // permet de lier les entity à leur composant en gardant les entites proches les unes des autres
		std::vector<Component> components; //contient les composants 
		std::vector<Entity> entities; //contient les entités qui ont ce composant  
		...
		void add(Entity entity, Component component){
			...
			if(!has(entity)){ //si l'entity n'as pas deja ce composant
				...
				components.push_back( std::move(component) ); //move permet de bouger completement le composant dans l'array pour ne pas qu'il soit supprimer si il sort du scope 
		...
		void remove(Entity entity){
			...
			std::uint32_t index = sparse[entity];

			Entity lastEntity = entities.back();

			//on le remplace
			entities[index] = lastEntity; 
			components[index] = std::move(components.back()); // utilisation de move pour que l'ecs marche avec des objets non copiables
	...
	class View{ //peut etre ameliorer
		...
		std::tuple<ComponentStorage<Components>*...> storages;  //un tuple de storage 

	public: 
		View( std::tuple<ComponentStorage<Components>*...>  storages_in){
		...
		class Iterator {
			public:
				using Iter = typename std::vector<Entity>::const_iterator;
			
				Iterator(Iter current, Iter end, const std::tuple<ComponentStorage<Components>*...>& storages): current(current), end(end), storages(storages) {
					skip_invalid();
				}
			...
			private:
				Iter current, end;
				const std::tuple<ComponentStorage<Components>*...> & storages;
	...
	*/
	...
	private:
		...
		std::vector<Entity> FreeIDs; //les id des entites qu'on a detruit pour pouvoir les reutiliser

		std::vector<IComponentStorage *> componentStorages; //permet de savoir quelle composant sont utilisé    
	...
	public:
		...
		Component & emplace(Entity entity, Arguments &&... arguments){ //ajoute un composant a une entite 
			...
			storage.add(entity , Component{ std::forward<Arguments>(arguments)... } ); //Component{ args } permet d'initialiser le composant
		...
		View<Components...> view() {
			return View<Components...>( std::make_tuple( &getComponentStorage<Components>()...) );
		}
