...
public:
    explicit PBRShader() {
        ...
        vert.addSource(Corrade::Containers::StringView{rs.getString("pbrShader.vs")});
        frag.addSource(Corrade::Containers::StringView{rs.getString("pbrShader.fs")});

        if(!vert.compile()) {
            Corrade::Utility::Error{} << "PBRShader: vertex shader compilation failed:\n";
        }
        if(!frag.compile()) {
            Corrade::Utility::Error{} << "PBRShader: fragment shader compilation failed:\n";
        }
    ...
    PBRShader& setModel(const Magnum::Matrix4& model) {
        setUniform(m_uModelLoc, model);
        return *this;
    }

    PBRShader& setMVP(const Magnum::Matrix4& mvp) {
        setUniform(m_uMVPLoc, mvp);
        return *this;
    }

    PBRShader& setLightVP(const Magnum::Matrix4& lightVP) {
        setUniform(m_uLightVPLoc, lightVP);
        return *this;
    }

    PBRShader& bindAlbedoTexture(Magnum::GL::Texture2D& albedo) {
        albedo.bind(1);
        setUniform(m_uAlbedoSamplerLoc, 1);
        return *this;
    }

    PBRShader& bindARM(Magnum::GL::Texture2D& arm) {
        arm.bind(2);
        setUniform(m_uARMSamplerLoc, 2);
        return *this;
    }

    PBRShader& bindNormalMap(Magnum::GL::Texture2D& normalMap) {
        normalMap.bind(3);
        setUniform(m_uNormalMapSamplerLoc, 3);
        return *this;
    }

    PBRShader& bindShadowMap(Magnum::GL::Texture2D& shadowMap) {
        shadowMap.bind(4);
        setUniform(m_uShadowMapSamplerLoc, 4);
        return *this;
    }

    PBRShader& setLightDirection(const Magnum::Vector3& dir) {
        setUniform(m_uLightDirectionLoc, dir);
        return *this;
    }
    PBRShader& setLightColor(const Magnum::Color3& color) {
        setUniform(m_uLightColorLoc, color);
        return *this;
    }
    PBRShader& setLightIntensity(float intensity) {
        setUniform(m_uLightIntensityLoc, intensity);
        return *this;
    }

    PBRShader& setUvScale(float scale) {
        setUniform(m_uUvScaleLoc, scale);
        return *this;
    }

    PBRShader& setCameraPosition(const Magnum::Vector3& pos) {
